...
inline PNT rot (PNT s,PNT t,PNT u,float angle)
{
...
//      exit(0);
   ...
   u.x = a2 * (k1 = cos(angle)) +  (c2 * b1 - b2 * c1) * (k2 = sqrt(1./r1) * sin(angle)) + m.x;
   u.y = b2 * k1 +  (a2 * c1 - c2 * a1) * k2 + m.y;
   u.z = c2 * k1 +  (b2 * a1 - a2 * b1) * k2 + m.z;
